...
export default function ProductInteractive({ product, settings }: { product: any, settings?: any }) {
  ...
  const currentVariant = useMemo(() => {
    ...
    return product.variants.find((v: any) => {
      const option1Match = !product.options?.[0] || v.option1 === selectedOptions[product.options[0].name];
      const option2Match = !product.options?.[1] || v.option2 === selectedOptions[product.options[1].name];
      const option3Match = !product.options?.[2] || v.option3 === selectedOptions[product.options[2].name];
      
      return option1Match && option2Match && option3Match;
    });
  }, [selectedOptions, product.variants, product.options]);

  const price = currentVariant?.price || product.variants?.[0]?.price || parseFloat(product.priceRange?.minVariantPrice?.amount || "0");
  const isAvailable = currentVariant?.inventory?.isAvailable ?? (product.variants?.[0]?.inventory?.isAvailable ?? true);
